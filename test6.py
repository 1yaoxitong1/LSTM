...
# 加载数据
series = read_csv('co.csv', header=0, parse_dates=[0], index_col=0,
                  date_parser=parser)
...
expected_values = raw_values[-500:]
expected_df = pd.DataFrame(expected_values, columns=['Expected'])
results_df = pd.concat([expected_df, predictions_df], axis=1)
